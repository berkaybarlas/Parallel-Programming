...
void simulate(double *E, double *E_prev, double *R,
    ...
    for (j = 1; j <= m; j++) {
        for (i = 1; i <= n; i++) {
            int index = j * (n + 2) + i;
            E[index] = E_prev[index] + alpha * (E_prev[index + 1] + E_prev[index - 1] - 4 * E_prev[index] +
                                                E_prev[index + m + 2] + E_prev[index - (m + 2)]);
        }
    }
    ...
    for (j = 1; j <= m; j++) {
        for (i = 1; i <= n; i++) {
            ...
            R[index] = R[index] + dt * (epsilon + M1 * R[index] / (E[index] + M2)) *
                                  (-R[index] - kk * E[index] * (E[index] - b - 1));
        }
    }
